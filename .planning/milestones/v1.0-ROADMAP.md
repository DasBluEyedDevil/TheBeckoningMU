...
This milestone takes the existing Evennia-based web portal — which already had working character creation, grid builder, and API endpoints — from a functional prototype to a production-ready system. The journey started with hardening existing code, then completed the character approval workflow, enhanced the builder editor UX, built the approval-to-sandbox-to-live pipeline for builder projects, and finished with the trigger system for dynamic room behavior. Seven phases deliver 28 requirements across five functional areas.
...
- Fixed CSRF vulnerabilities in all API endpoints (removed 11 @csrf_exempt decorators)
- Fixed @py injection vulnerability in builder exporter
- Added optimistic concurrency control to prevent data overwrites in builder
- Implemented server-side V5 pool validation for character creation
- Resolved Python 3.14/cryptography incompatibility (documented, requires Python 3.12/3.13)

**Issues Deferred:**

- Up/Down vertical exit support in compass system (Phase 3 gap — minor, 2D directions cover majority of use cases)
- Human E2E testing for character approval workflow (environment blocker — code verified correct)

**Technical Debt Incurred:**

- Up/Down (U/D) vertical exits not implemented — grid coordinate system needs z-axis support
- Minor code duplication in scripts.py is_valid method
- In-clan discipline validation deferred (needs ClanDiscipline model not in current schema)
