...
must_haves:
  truths:
    ...
    - "CharacterCreateAPI uses enhanced_import_character_from_json which runs pool validation"
  artifacts:
...
<task type="auto">
  <name>Task 1a: Remove all @csrf_exempt decorators and add basic authentication checks</name>
...
Then add authentication checks to GET endpoints that currently have NO auth:
...
Grep for `is_authenticated` in TraitCategoriesAPI, TraitsAPI, DisciplinePowersAPI, CharacterValidationAPI -- each should have a check.
...
4 GET/catalog endpoints have authentication checks.
Error messages in CharacterImportAPI and CharacterCreateAPI use generic text.
  </done>
</task>

<task type="auto">
  <name>Task 1b: Add ownership checks, staff gates, and idempotent approval</name>
  <files>beckonmu/traits/api.py</files>
  <action>
Add authorization and special-case checks to the remaining 3 endpoints that need more than basic auth:

1. **CharacterImportAPI.post()** -- Add BOTH auth check AND staff check: if not request.user.is_authenticated: return 401. if not request.user.is_staff: return JsonResponse({'error': 'Staff permissions required'}, status=403). This is the CRITICAL fix -- currently any anonymous user can overwrite character traits.

2. **CharacterExportAPI.get()** -- Add auth check, then add ownership/staff check: after fetching the character, verify request.user is the character's db_account OR is_staff. Return 403 if neither.

3. **CharacterAvailableTraitsAPI.get()** -- Same ownership/staff pattern as CharacterExportAPI.

Additionally, verify that auth checks exist BEFORE the existing staff check logic in these endpoints (which already check is_authenticated AND is_staff):
- PendingCharactersAPI -- OK as-is
- CharacterDetailAPI -- OK as-is
...
The best place to add auth checks is in the individual get/post methods (not in dispatch or via mixin) to match the existing pattern used by PendingCharactersAPI and CharacterDetailAPI. This keeps the codebase consistent.
  ...
  <verify>
Grep for `is_staff` in CharacterImportAPI -- should appear.
Grep for `db_account` in CharacterExportAPI -- should appear (ownership check).
Grep for `already approved` in CharacterApprovalAPI -- should appear.
Grep for `is_authenticated` in traits/api.py -- should appear at least 7 times total (4 from Task 1a + 3 from Task 1b).
  </verify>
  <done>
...
All 7 previously-unprotected endpoints now require authentication.
  </done>
</task>
...
Verify the wiring from CharacterCreateAPI to pool validation:
`grep -n "enhanced_import_character_from_json" beckonmu/traits/api.py` -- should confirm CharacterCreateAPI uses this code path (expected in the post() method of CharacterCreateAPI). This grep verifies the key_link from api.py to utils.py is intact.
  </verify>
  <done>
...
CharacterCreateAPI confirmed to call enhanced_import_character_from_json (key_link verified by grep).
  </done>
</task>
...
8. `grep -n "enhanced_import_character_from_json" beckonmu/traits/api.py` confirms CharacterCreateAPI uses this validation path
</verification>
...
- CharacterCreateAPI confirmed to use enhanced_import_character_from_json (key_link verified)
</success_criteria>
