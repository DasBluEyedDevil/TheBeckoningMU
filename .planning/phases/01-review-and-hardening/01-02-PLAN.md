...
must_haves:
  truths:
    ...
    - "Saving a project increments its version number"
    - "Saving with a stale version number returns 409 Conflict"
    - "map_data includes schema_version field on new projects"
    - "Validator checks room data shape (name field exists)"
  artifacts:
