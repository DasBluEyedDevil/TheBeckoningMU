...
<task type="auto">
  <name>Task 2: Add edit mode and resubmission flow to character_creation.html</name>
...
- Check if there is an existing `DOMContentLoaded` listener or an init function -- add edit mode loading to the existing one rather than creating a second listener.
...
Functional verification (run these after changes):
- Run JavaScript syntax check: `cd C:/Users/dasbl/PycharmProjects/TheBeckoningMU && .venv/Scripts/python.exe -c "import re; content=open('beckonmu/web/templates/character_creation.html').read(); scripts=re.findall(r'<script[^>]*>(.*?)</script>', content, re.DOTALL); print(f'Found {len(scripts)} script blocks, total {sum(len(s) for s in scripts)} chars')"` -- should find script blocks without read errors
- Start the Evennia server and verify page loads without JS console errors: `cd C:/Users/dasbl/PycharmProjects/TheBeckoningMU && .venv/Scripts/python.exe -c "import urllib.request; r=urllib.request.urlopen('http://localhost:4001/character-creation/'); print(f'Status: {r.status}, Length: {len(r.read())}')"` -- should return 200 (only if server is running; skip if server is not running)
- Verify no duplicate function definitions: search for duplicate `function loadCharacterForEdit` or `function populateFormFromCharacterData` -- each should appear exactly once
  </verify>
  <done>Character creation form has: background/backstory textarea included in form submission (CHAR-02), edit mode that loads rejected character data and shows rejection notes banner (CHAR-01), resubmission flow that POSTs to /resubmit/ endpoint (CHAR-01).</done>
</task>

<task type="auto">
  <name>Task 3: Add draft save/resume via localStorage to character_creation.html</name>
  <files>beckonmu/web/templates/character_creation.html</files>
  <action>
Continue editing `beckonmu/web/templates/character_creation.html` (same file as Task 2). This task adds the draft persistence feature (CHAR-05).
...
- Search for `setInterval.*saveDraft` or `setInterval(saveDraft` -- auto-save timer exists
- Search for `applyDraftToForm` -- function exists

Functional verification:
- Run JavaScript syntax check: `cd C:/Users/dasbl/PycharmProjects/TheBeckoningMU && .venv/Scripts/python.exe -c "import re; content=open('beckonmu/web/templates/character_creation.html').read(); scripts=re.findall(r'<script[^>]*>(.*?)</script>', content, re.DOTALL); print(f'Found {len(scripts)} script blocks, total {sum(len(s) for s in scripts)} chars'); [print(f'  Block {i+1}: {len(s)} chars, starts with: {s.strip()[:80]}...') for i,s in enumerate(scripts)]"` -- should show script blocks with reasonable sizes
- Verify no syntax-breaking issues by checking paired braces: `cd C:/Users/dasbl/PycharmProjects/TheBeckoningMU && .venv/Scripts/python.exe -c "c=open('beckonmu/web/templates/character_creation.html').read(); print(f'Open braces: {c.count(chr(123))}, Close braces: {c.count(chr(125))}')"` -- counts should be equal (or very close, accounting for template tags)
- Verify no duplicate function definitions: search for multiple occurrences of `function saveDraft` or `function loadDraft` -- each should appear exactly once
  </verify>
  <done>Character creation form has: draft auto-save to localStorage every 30s (CHAR-05), draft resume prompt on page load for non-edit mode (CHAR-05), draft cleared on successful submit (CHAR-05), drafts keyed by character ID to prevent cross-contamination (CHAR-05).</done>
</task>
